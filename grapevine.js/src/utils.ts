...
export const getParams = async (parallel = true,): Promise<Blob> => {
    ...
    for (let i = 0; i < NUM_PARAMS_CHUNKS; i++) {
        ...
        const request = fetchWithRetry(uri, { headers: { contentType: "application/x-binary" } });
 ...
 */
export const initGrapevineWasm = async (threads = 1): Promise<GrapevineWasm> => {
    const wasm = await import("../grapevine_wasm/pkg/grapevine_wasm");
    await wasm.default();
    if (typeof navigator !== "undefined" && threads > 1) {
        let concurrency = navigator.hardwareConcurrency;
        if (threads > concurrency)
            console.warn(`Requested ${threads} threads but only ${concurrency} available`);
        else 
            concurrency = threads;
